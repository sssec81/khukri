...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("khukri_engine=info".parse().unwrap()))
        .init();
    ...
    if args.len() < 3 {
        ...
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  cargo run --example download -- https://speed.hetzner.de/100MB.bin /tmp/test.bin");
    ...
    let config = DownloadConfig {
        ...
        throttle: ThrottleConfig { bytes_per_sec: speed_limit },
    };
